...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_update_recipe_assign_tag(self):
        """Assiging an existing tag when updating a recipe"""
    ...
    def create_recipe_with_new_ingredients(self):
        ...
        payload = {
            'title': 'Califlouer Tacos',
            "time+_minutes": 60,
            'price': Decimal('4.30'),
            'ingredients': [{'name': 'Califlouer'}, {'name': 'Salt'}]
        }
        res = self.client.post(RECIPES_URL, payload, format="json")
    ...
    def test_update_recipe_assign_ingredient(self):
        """Assiging an existing ingredient when updating a recipe"""
    ...
    def test_filter_by_ingredients(self):
        """Test filter recipes by test_filter_by_ingredients"""
        r1 = create_recipe(user=self.user, title='Posh Beans on Toast')
        r2 = create_recipe(user=self.user, title='Chicen with Tahini')
        ing1 = Ingredient.objects.create(user=self.user, name='Feta cheese')
        ing2 = Ingredient.objects.create(user=self.user, name='Gouda')
